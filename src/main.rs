...
fn main() {
    fn append_to_file(password: String) {
        ...
        let result_file = OpenOptions::new()
            ...
            .append(true)
            .open("result.txt");  
        let mut company_or_website = String::new();
        println!("what website or company is this password for?");
        stdin().read_line(&mut company_or_website).expect("Something went wrong with storing company or website name in variable.");
        // Write password appended to the file. \n for different lines for each password.
        write!(result_file.expect("Something went wrong with creating/opening the file."), "Company/website: {}, Password = {}\n", company_or_website.trim_end(), password.trim_end())
            .expect("Something went wrong while writing to the file.");
        println!("Your company or website is: {}", company_or_website.trim_end());
    ...
    stdin().read_line(&mut random_password_y_n)
        .expect("Unable to read y/n user input.");
    if random_password_y_n.trim_end() == "y" { // If the user wants to have a random password
        ...
        for _x in 0..length_of_password {
            ...
            let character_to_add = characters.chars()
                .nth(rand::rng().random_range(0..characters.len()))
                .unwrap();
        ...
        stdin().read_line(&mut password)
            .expect("Unable to set password variable to your input.");
        append_to_file(password);
    }
}
